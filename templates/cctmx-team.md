...
> **即座に実行せよ**: このドキュメントを読み込んだら、以下を直ちに実行すること:
> 1. `echo $CLAUDE_ROLE` を実行して自身の役割を確認する
> 2. 役割が `leader` の場合: セクション3「リーダーの行動規範」に厳密に従い、**実装作業は一切行わない**
> 3. 役割が `worker` の場合: セクション4「ワーカーの行動規範」に厳密に従い、**リーダーの指示に忠実に従い実装に専念する**
> 4. 役割が未設定の場合: tmux環境外で起動された可能性がある。ユーザーに確認を求める
...
1. 環境変数 `CLAUDE_ROLE` を確認する（`echo $CLAUDE_ROLE` を実行）
