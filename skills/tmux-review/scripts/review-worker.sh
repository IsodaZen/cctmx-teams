#!/bin/bash
set -euo pipefail

# ワーカーペイン情報の確認
worker_info_file="${CLAUDE_PROJECT_DIR}/.claude/worker-info"
if [ ! -f "$worker_info_file" ]; then
  echo "❌ エラー: ワーカーペイン情報が見つかりません" >&2
  echo "/tmux-worker スキルを先に実行してください" >&2
  exit 1
fi

# ワーカーペイン情報を読み込み
# shellcheck source=/dev/null
source "$worker_info_file"

if [ -z "${CLAUDE_WORKER_PANE:-}" ] || [ -z "${CLAUDE_WORKER_SESSION:-}" ]; then
  echo "❌ エラー: ワーカーペイン情報が不正です" >&2
  exit 1
fi

session="${CLAUDE_WORKER_SESSION}"
worker_pane="${CLAUDE_WORKER_PANE}"

echo "📊 ワーカーの出力をキャプチャ中..." >&2
echo "セッション: ${session}" >&2
echo "ペイン: ${worker_pane}" >&2
echo "" >&2

# ワーカーの出力をキャプチャ
if ! output=$(tmux capture-pane -t "${session}:${worker_pane}" -p -S -3000 2>&1); then
  echo "❌ エラー: ワーカーペインの出力を取得できませんでした" >&2
  echo "$output" >&2
  exit 1
fi

# 出力を一時ファイルに保存（SCRATCHPAD_DIRを使用）
output_file="${SCRATCHPAD_DIR}/worker-output-$(date +%s).txt"
echo "$output" > "$output_file"

echo "✅ ワーカーの出力を保存しました: ${output_file}" >&2
echo "" >&2
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━" >&2
echo "ワーカーの出力:" >&2
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━" >&2
echo "$output"
echo "" >&2
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━" >&2
echo "📋 レビューチェックリスト:" >&2
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━" >&2
echo "" >&2
echo "共通チェック項目:" >&2
echo "- [ ] タスクの目的が達成されているか" >&2
echo "- [ ] 要件がすべて満たされているか" >&2
echo "- [ ] 制約条件が守られているか" >&2
echo "- [ ] コードの品質（可読性、保守性）は適切か" >&2
echo "- [ ] エラーハンドリングが適切に実装されているか" >&2
echo "- [ ] TypeScript型エラーがないか" >&2
echo "- [ ] ESLintエラーがないか" >&2
echo "- [ ] ビルドが成功するか" >&2
echo "" >&2
echo "タスクタイプ別チェック:" >&2
echo "" >&2
echo "【機能追加の場合】" >&2
echo "- [ ] 既存機能への影響がないか" >&2
echo "- [ ] ドキュメントが更新されているか" >&2
echo "- [ ] 型定義が適切か" >&2
echo "- [ ] UIの一貫性が保たれているか" >&2
echo "" >&2
echo "【バグ修正の場合】" >&2
echo "- [ ] 根本原因が解決されているか" >&2
echo "- [ ] 同様のバグが他にないか" >&2
echo "- [ ] 回帰テストが追加されているか" >&2
echo "- [ ] 副作用がないか" >&2
echo "" >&2
echo "【リファクタリングの場合】" >&2
echo "- [ ] 動作が変わっていないか" >&2
echo "- [ ] パフォーマンスが劣化していないか" >&2
echo "- [ ] すべてのテストが通るか" >&2
echo "" >&2
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━" >&2
echo "次のステップ:" >&2
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━" >&2
echo "- 問題があれば修正指示を作成" >&2
echo "- 問題なければマネージャーに報告" >&2
echo "" >&2

exit 0
