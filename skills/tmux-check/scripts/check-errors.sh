#!/bin/bash
set -euo pipefail

# ワーカーペイン情報の確認
worker_info_file="${CLAUDE_PROJECT_DIR}/.claude/worker-info"
if [ ! -f "$worker_info_file" ]; then
  echo "❌ エラー: ワーカーペイン情報が見つかりません" >&2
  echo "/tmux-worker スキルを先に実行してください" >&2
  exit 1
fi

# ワーカーペイン情報を読み込み
# shellcheck source=/dev/null
source "$worker_info_file"

if [ -z "${CLAUDE_WORKER_SESSION:-}" ] || [ -z "${CLAUDE_WORKER_WINDOW:-}" ] || [ -z "${CLAUDE_WORKER_PANE:-}" ]; then
  echo "❌ エラー: ワーカーペイン情報が不正です" >&2
  echo "worker-infoの内容:" >&2
  cat "$worker_info_file" >&2
  exit 1
fi

session="${CLAUDE_WORKER_SESSION}"
tmux_target="${session}:${CLAUDE_WORKER_WINDOW}.${CLAUDE_WORKER_PANE}"

echo "🔍 ワーカーのエラーをチェック中..." >&2
echo "ターゲット: ${tmux_target}" >&2
echo "" >&2

# ワーカーの出力をキャプチャ
if ! output=$(tmux capture-pane -t "${tmux_target}" -p -S -3000 2>&1); then
  echo "❌ エラー: ワーカーペインの出力を取得できませんでした" >&2
  echo "ターゲット: ${tmux_target}" >&2
  echo "$output" >&2
  exit 1
fi

# エラー検知（大文字小文字を区別しない）
error_lines=$(echo "$output" | grep -iE "(error|エラー|失敗|exception|fatal)" || true)

if [ -n "$error_lines" ]; then
  echo "⚠️ ワーカーでエラーが発生している可能性があります" >&2
  echo "" >&2
  echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━" >&2
  echo "エラーメッセージ:" >&2
  echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━" >&2
  echo "$error_lines" | tail -20
  echo "" >&2
  echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━" >&2
  echo "リーダーの対応フロー:" >&2
  echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━" >&2
  echo "1. エラー内容を分析" >&2
  echo "   - エラーメッセージの内容" >&2
  echo "   - 発生箇所（ファイル、行番号）" >&2
  echo "   - スタックトレース" >&2
  echo "" >&2
  echo "2. マネージャーに報告" >&2
  echo "   - エラー詳細" >&2
  echo "   - 推定原因" >&2
  echo "   - 影響範囲" >&2
  echo "" >&2
  echo "3. 修正方針を相談" >&2
  echo "   - マネージャーと修正方針を議論" >&2
  echo "   - 必要に応じて要件を再確認" >&2
  echo "" >&2
  echo "4. ワーカーに再指示" >&2
  echo "   - 修正内容を構造化指示フォーマットで送信" >&2
  echo "" >&2

  exit 1
fi

echo "✅ エラーは検出されませんでした" >&2
echo "" >&2
echo "ワーカーは正常に動作しています。" >&2
echo "" >&2
echo "次のステップ:" >&2
echo "- タスク完了後に /tmux-review でレビューを実施" >&2
echo "- 定期的に /tmux-check で監視を継続" >&2
echo "" >&2

exit 0
